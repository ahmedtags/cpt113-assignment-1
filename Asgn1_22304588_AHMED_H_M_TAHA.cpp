...
public:
    ...
    int calculate_service_years(int end_year) const {
        return end_year - Appointment_Year;
    }

    int calculate_service_months(int end_year, int end_month) const {
        int total_months = (end_year - Appointment_Year) * 12 + (end_month - Appointment_Month);
         if (total_months > 360) {
       return 360;
      } else {
        return total_months;
...
public:
    ...
    void calculate_eligibility() const {
...
int main() {
    Pension_Info emp;
    char choice;
    bool personalInfoEntered = false, serviceInfoEntered = false;
